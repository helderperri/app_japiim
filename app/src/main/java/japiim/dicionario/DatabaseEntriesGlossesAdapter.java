...
public class DatabaseEntriesGlossesAdapter {
    ...
    public DatabaseEntriesGlossesAdapter(Context context, long entryBundleId, long senseBundleIid, long senseId) {
    ...
    public List<GetEntriesGlossesTableValues> getEntriesGlosses(){
        ...
        while(cursor.moveToNext()){
            ...
            int index12 = cursor.getColumnIndex(DatabaseHelper.KEY_GLYPHID);
            int glyphId = cursor.getInt(index12);

            GetEntriesGlossesTableValues getEntriesGlossesTableValues = new GetEntriesGlossesTableValues(entryBundleId, entryId, senseBundleId, senseId, senseOrder, targetLang, langCode, gloss, classId, className, glyphId);
            entriesGlossesList.add(getEntriesGlossesTableValues);
    ...
    public List<GetEntriesGlossesTableValues> getGlossessToSearchDisplay(){
    ...
    public List<GetEntriesGlossesTableValues> getGlossessToSearchDisplayByGlyph(String glyph){
        Cursor cursor = db.rawQuery("SELECT * FROM " + DatabaseHelper.TABLE_NAME + " WHERE gloss LIKE '" + glyph + "%' ORDER BY " + DatabaseHelper.KEY_GLOSS + " COLLATE UNICODE", null);
        //StringBuffer buffer = new StringBuffer();
    ...
    public List<GetEntriesGlossesTableValues> getGlossessToSearchDisplayByGlyphId(long glyphId){
        Cursor cursor = db.rawQuery("SELECT * FROM " + DatabaseHelper.TABLE_NAME + " WHERE glyph_id = " + glyphId + " ORDER BY " + DatabaseHelper.KEY_GLOSS + " COLLATE UNICODE", null);
        //StringBuffer buffer = new StringBuffer();
        ...
        while(cursor.moveToNext()){
            ...
            String className = cursor.getString(index11);
            int index12 = cursor.getColumnIndex(DatabaseHelper.KEY_GLYPHID);
            int glyphId2 = cursor.getInt(index12);

            GetEntriesGlossesTableValues getEntriesGlossesTableValues = new GetEntriesGlossesTableValues(entryBundleId, entryId, senseBundleId, senseId, senseOrder, targetLang, langCode, gloss, classId, className, glyphId2);
            entriesGlossesList.add(getEntriesGlossesTableValues);
    ...
    private static class DatabaseHelper extends SQLiteOpenHelper {
        ...
        private static final String TABLE_NAME = "entries_glosses";
        ...
        private static final String CREATE_TABLE = "create table "+ TABLE_NAME+
                " ("+ KEY_ENTRYBUNDLEID+" integer, "+ KEY_ENTRYID+" integer, "+ KEY_SENSEBUNDLEID+" integer, " + KEY_GLOSSID + " integer, "+ KEY_ENTRYGLOSSID+" integer primary key autoincrement, " + KEY_GLOSSORDER + " integer, " + KEY_TARGETLANG + " integer, " + KEY_LANGCODE + " text, " + KEY_GLOSS + " text, " + KEY_CLASSID + " integer, " + KEY_CLASSNAME + " text, " + KEY_GLYPHID + " integer)";
